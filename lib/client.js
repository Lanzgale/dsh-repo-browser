...
.fe-rb-repo-priv {
  ...
  padding: 0 5px; border: 1px solid var(--fe-border-2); border-radius: 8px;
...
`;
		...
		// ---------- api ----------
		const api = {
			list: () => fetch('/plugins/repo-browser/list').then((r) => r.json()),
		...
		const RepoBrowserPanel = (props) => {
			...
			const visibleRepos = selectedGroup === '__all__'
				? (repos || [])
			...
			const renderGroupList = () => {
				const rows = [];
				const builtins = [
					{ id: '__all__', label: '全部仓库', count: (repos || []).length },
					{ id: '__ungrouped__', label: '未分类', count: ungrouped.length },
				];
				for (const b of builtins) {
					rows.push(react.createElement('div', {
						key: b.id,
						className: 'fe-rb-group' + (selectedGroup === b.id ? ' fe-rb-group-selected' : ''),
						onClick: () => setSelectedGroup(b.id),
					},
						react.createElement(Icon, { name: b.id === '__all__' ? 'star' : 'folder', size: 13 }),
						react.createElement('span', { style: { overflow: 'hidden', textOverflow: 'ellipsis' } }, b.label),
						react.createElement('span', { className: 'fe-rb-count' }, b.count),
					));
				}
			...
			const renderRepo = (r) => react.createElement('div', {
				...
				react.createElement('div', { className: 'fe-rb-repo-top' },
					react.createElement('span', { className: 'fe-rb-repo-name' },
						r.name,
						r.private ? react.createElement('span', { className: 'fe-rb-repo-priv' }, ' 私有') : null,
					),
				...
				react.createElement('div', { className: 'fe-rb-repo-meta' },
					...
					r.archived ? react.createElement('span', { style: { color: 'var(--fe-err)' } }, '归档') : null,
				),
				react.createElement('button', {
